...
| **42** | glTF models built by the pipeline: 14 creation characters + 25 monsters + 3 NPCs |
| **29,812** | skill records decoded from the encrypted `.dat` tables (~93k records across 10 files) |
...
- **ElberaGate — the protocol bridge.** One real TCP session per browser:
  ...
  attack in; world state, movement, chat, combat events out. Device-id
  ...
  *Verify:* `cd gateway && node test/verify-one.js` (plus `verify-two`,
  `verify-combat`, `verify-observer` — all PASS against the live server).
- **ElberaClient — the walkable world.** three.js world streaming over the
  ...
  option, nameplates, chat, target frame, HP/MP bars, damage floats,
  death/revive. Solo offline or online through ElberaGate.
...
- **ElberaDat — the table decoders.** 10 RSA-encrypted game-data files →
  JSON (~93k records: npcs, items, skills, names, UI strings), every parser
  asserting exact byte consumption.
...
- **Real protocol gateway.** Live sessions on the unmodified server: NPCs,
  players, movement, chat, combat — `gateway/test/verify-*.js` all PASS.
...
- **42 production models**, structurally validated, numerically diffed
...
- **The browser is a renderer, not the game.** All logic lives in aCis; the
  client implements the protocol subset in the gateway contract. No skills,
  inventory or quests in the browser yet (roadmap M4).
- **Identity by possession.** Your account is a device id in
  `localStorage` — clearing browser data loses the character (recovery
  codes are on the roadmap). The gateway is plaintext localhost WebSocket,
...
- **Simplified terrain texturing** in spots (preview basecolor blend; exact
  ...
  water, particles and baked lighting are not extracted. Dungeon-interior
  tiles (19_16, 21_25) convert correctly but need a client interior mode.
- **`all` chat is radius-limited to 1,250 units by the server itself**;
  `SystemMessage` is decoded to the gateway log, not the UI yet.
- **Deploy**: arm64 verified, amd64 untested; retail-client login through
  Docker untested.

## Roadmap

Milestones M1–M3 of the [master plan](docs/web-port-architecture.md) are
done; combat (M4's core) landed early.

- **M4 — skills & items.** `MagicSkillUse`/`MagicSkillLaunched`, casting
  UI, `ItemList`/`InventoryUpdate` with icons from the texture library,
  loot flow, `SystemMessage` surfaced in the UI.
- **M5 — chat & core UI.** All chat channels, inventory/character-sheet
  windows, hotbar, `.menu` passthrough (server mod already exists),
  account recovery codes, WASD refinement.
- **After M5.** KTX2 textures, WebGPU evaluation, mobile layout,
  dungeon-interior mode, Seven Signs tiles, full-library HD upscale,
  public VPS deployment ([runbook](docs/README-ADMIN.md), ES).
...
mundo, 42 modelos 3D con sus animaciones, más de 30 mil texturas— se
...
interiores y la apertura pública en un VPS. Ojo: tu cuenta vive en el
navegador — si borras sus datos, pierdes el acceso a tu personaje.
