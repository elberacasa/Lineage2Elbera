...
> | "verify_m5 fails on `.click` of undefined" / "verify_targetwnd cannot pass under a battery" (this wave's brief) | Both **PASS** in the full sweep of 2026-08-09: `verify_m5` 16 s, `verify_targetwnd` 33 s. Fixed in an earlier wave; the brief was repeating a stale commit message |
> | verify_skillanim's fx checks — "the melee slash is #ffc060, the projectile #6fd8ff, the self-heal aura #86f0b0" | Every one of those was an **authored colour**, and the suite could not see the real effects at all: its `fxNow()` filtered to `fx.kind != null`, which keeps ONLY the authored-pop objects. SkillVfx tags its retail objects `skillvfx.json` / `skillmesh.json` and gives them no `kind`. So when the invented pops were deleted as unsourced (`js/skills.js:319` — `flash()` now returns without drawing), all five checks read `[]` and went red **because the port got more correct**. Rewritten 2026-08-09 to assert provenance: the strike now verifiably draws the decoded retail meshes `skill_power01` (skill 3) and `windblowin00` (1177), and skill **1216 — which is absent from skillvfx.json's 290-entry table — must draw nothing**. 9/9 pass |
> | "verify_app is a hung suite" | It was not hung *doing* anything: it completed every measurement, printed its full summary, and then sat at 0.0% CPU until the watchdog killed it at 300 s. `browser.close()` had already returned (the summary prints after the `finally`). A handle outlived the browser and nothing called `process.exit`. With an explicit flush-then-exit the same suite finishes in **47 s** — so 270 of those 317 seconds were pure hang, and the *only* reason it showed up as a row instead of eating the sweep is the watchdog |
> | "the live browser suites are failing against the real stack" | **Eight of them are suite bugs, not product bugs, and the product is behaving correctly.** Each headless run gets a fresh Chrome profile → no stored `l2vzla.deviceId` → the client mints a new one → a NEW account → `auth_ok` comes back with `chars: 0` → the client opens the character-creation overlay and waits, exactly as it should. The suites then time out after 120 s waiting for an `enterWorld` that can never come. Correlation is perfect over the sweep: every live suite that seeds a deviceId (`verify_clanwnd_live`, `verify_warehousewnd_live`, `verify_soulshot`, `verify_equipswap`) PASSED; every one that does not (`verify_live`, `verify_minimap_live`, `verify_actionwnd_live`, `verify_partywnd_live`, `verify_questwnd_live`, `verify_storewnd_live`, `verify_skilldepth_live`, `verify_tradewnd_live`) FAILED. Reproduced standalone on an idle machine, so it is not load. **A near miss worth recording**: the first probe read `charCreate.visible`, got `false`, and pointed at a severe product defect — a new player unable to enter the game. The property is `charCreate.open`, and it is `true`. One wrong property name away from filing a critical bug against correct code |
>
